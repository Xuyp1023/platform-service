...
public class NotificationProvider {
    ...
    public String selectNotificationProfileByCustsSql(final Map<String, Object> anParam) {
        ...
        final StringBuilder sql = new StringBuilder("SELECT np.C_PROFILE_NAME AS profileName, np.C_PROFILE_RULE AS profileRule, c.L_CUSTNO AS custNo, c.C_CUSTNAME AS custName ");
        sql.append(" FROM t_sys_notifi_profile np ").append(" INNER JOIN (SELECT * FROM t_custinfo c_1 WHERE c_1.L_CUSTNO IN (");
        sql.append(customer);
        sql.append(")) c ON np.C_PROFILE_RULE IN (SELECT ccr.C_RULE FROM t_cust_certinfo_rule ccr WHERE ccr.C_SERIALNO IN (SELECT cc.C_SERIALNO FROM t_cust_certinfo cc WHERE  cc.C_OPERORG = c.C_OPERORG))");
        sql.append(" AND np.C_CUSTOM != '1' ");
        sql.append(" WHERE np.C_SUBSCRIBE_ENABLE = '0' AND ( ");
        sql.append(" EXISTS ( SELECT np_1.ID  FROM t_sys_notifi_profile np_1 WHERE np_1.C_PROFILE_NAME = np.C_PROFILE_NAME AND np_1.C_PROFILE_RULE = np.C_PROFILE_RULE AND np_1.L_CUSTNO = c.L_CUSTNO AND np_1.C_CUSTOM = '1' AND np_1.C_BUSIN_STATUS = '1' ");
        sql.append(" ) OR NOT EXISTS ( SELECT np_1.ID FROM t_sys_notifi_profile np_1 WHERE np_1.C_PROFILE_NAME = np.C_PROFILE_NAME AND np_1.C_PROFILE_RULE = np.C_PROFILE_RULE AND np_1.L_CUSTNO = c.L_CUSTNO AND np_1.C_CUSTOM = '1' ))");
