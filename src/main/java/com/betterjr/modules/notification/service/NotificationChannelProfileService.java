...
@Service
public class NotificationChannelProfileService
        extends BaseService<NotificationChannelProfileMapper, NotificationChannelProfile> {
     ...
     * @return
     */
    public NotificationChannelProfile findChannelProfileByProfileIdAndChannel(final Long anProfileId,
     ...
     * @param anContentText
     */
    public NotificationChannelProfile saveChannelProfile(final NotificationChannelProfile anChannelProfile,
            final Long anChannelProfileId, final Long anCustNo, final String anContentText) {
        ...
        if (profileService.checkDefaultProfile(tempProfile) == true) { // 找到消息模板为 default模板
            ...
            if (profileService.checkDefaultProfile(tempProfile) == true) { // 找到消息模板为 default模板
                ...
                channelProfile = saveModifyChannelProfile(anChannelProfile, tempProfile,
                        tempChannelProfile.getChannel(), anCustNo, anContentText);
            } else { // 找到消息模板为 custom模板
                channelProfile = saveModifyChannelProfile(anChannelProfile, tempProfile,
                        tempChannelProfile.getChannel(), anCustNo, anContentText);
            }
        } else { // 找到消息模板为 custom模板
            channelProfile = saveModifyChannelProfile(anChannelProfile, tempProfile, tempChannelProfile.getChannel(),
                    anCustNo, anContentText);
        }
    ...
    private NotificationChannelProfile saveModifyChannelProfile(final NotificationChannelProfile anChannelProfile,
            ...
            final String anContentText) {

        final NotificationChannelProfile tempChannelProfile = findChannelProfileByProfileIdAndChannel(anProfile.getId(),
                anChannel);
        // 修改状态
        ...
        try {
            ...
            case "1":
                ...
                break;
            case "2":
                tempChannelProfile.setContent(
                        anContentText != null ? anContentText : resolveContent(anChannelProfile.getContent()));
                break;
            }
        }
     ...
     * 将基础数据copy到新的数据上
     */
    public void saveCopyBaseDataToTargetData(final Long anBaseProfileId, final Long anTargetProfileId,
            final CustInfo anCustInfo, final CustOperatorInfo anOperator) {
        ...
        for (final NotificationChannelProfile channelProfile : channelProfiles) {
            ...
            profileVariableService.saveCopyBaseDataToTargetData(channelProfile.getId(), tempChannelProfile.getId(),
                    anCustInfo, anOperator);
        }
    }
}
