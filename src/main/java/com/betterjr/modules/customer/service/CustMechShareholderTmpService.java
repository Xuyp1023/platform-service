...
public class CustMechShareholderTmpService extends BaseService<CustMechShareholderTmpMapper, CustMechShareholderTmp>
     ...
     * 添加新增变更流水记录
     */
    public CustMechShareholderTmp addChangeShareholderTmp(final CustMechShareholderTmp anShareholderTmp,
     ...
     * 添加修改变更记录
     */
    public CustMechShareholderTmp saveSaveChangeShareholderTmp(final CustMechShareholderTmp anShareholderTmp,
            final String anFileList) {
        BTAssert.notNull(anShareholderTmp, "公司股东流水信息不允许为空！");
        ...
        final CustMechShareholderTmp tempShareholderTmp = findShareholderTmpByRefId(refId,
        ...
        if (tempShareholderTmp == null) {
            ...
            anShareholderTmp.setBatchNo(fileItemService.updateAndDuplicateConflictFileItemInfo(anFileList,
                    anShareholderTmp.getBatchNo(), UserUtils.getOperatorInfo()));
            return addShareholderTmp(anShareholderTmp);
        } else {
            ...
            tempShareholderTmp.setBatchNo(fileItemService.updateAndDuplicateConflictFileItemInfo(anFileList,
                    tempShareholderTmp.getBatchNo(), UserUtils.getOperatorInfo()));
    ...
    public int saveCancelChangeShareholderTmp(final Long anId) {
        ...
        if (StringUtils.equals(shareholderTmp.getTmpType(), CustomerConstants.TMP_TYPE_CHANGE) == false) {
            throw new BytterTradeException("流水信息类型不正确,不可撤销.");
        }

        if (StringUtils.equals(shareholderTmp.getBusinStatus(), CustomerConstants.TMP_STATUS_NEW) == false) {
    ...
    public Collection<CustMechShareholderTmp> queryCustMechShareholderTmpByChangeApply(final Long anApplyId) {
        ...
        final List<Long> tmpIds = COMMA_PATTERN.splitAsStream(changeApply.getTmpIds()).map(Long::valueOf)
    ...
    public CustChangeApply addChangeApply(final Map<String, Object> anParam, final Long anCustNo) {
        ...
        final CustChangeApply changeApply = changeApplyService.addChangeApply(anCustNo,
                CustomerConstants.ITEM_SHAREHOLDER, tempTmpIds);
        ...
        final Collection<CustMechShareholderTmp> shareholderTmps = this.selectByProperty("parentId",
                changeApply.getId());
    ...
    private void saveNormalShareholders(final Collection<CustMechShareholder> anShareholders,
            final Collection<CustMechShareholderTmp> anShareholderTmps, final CustChangeApply anChangeApply,
            final String anTmpType) {
        final Long parentId = anChangeApply.getId();

        anShareholders.stream().filter(shareholder -> checkIsNormalShareholder(shareholder, anShareholderTmps) == true)
                .forEach(shareholder -> {
     ...
     * 检查是否是未改变股东
     */
    private boolean checkIsNormalShareholder(final CustMechShareholder anShareholder,
            final Collection<CustMechShareholderTmp> anShareholderTmps) {
        boolean flag = true;
     ...
     * 修改公司股东流水信息
     */
    public CustMechShareholderTmp saveShareholderTmp(final CustMechShareholderTmp anShareholderTmp, final Long anId,
            final String anFileList) {
        BTAssert.notNull(anShareholderTmp, "公司股东流水信息不允许为空！");
        ...
        tempShareholderTmp.initModifyValue(anShareholderTmp);
        tempShareholderTmp
                .setBatchNo(fileItemService.updateCustFileItemInfo(anFileList, tempShareholderTmp.getBatchNo()));
        return saveShareholderTmp(tempShareholderTmp);
    }
     ...
     * 保存 parentId 和 状态
     */
    public CustMechShareholderTmp saveShareholderTmpParentIdAndStatus(final Long anId, final Long anParentId,
     ...
     */
    public CustMechShareholderTmp addInsteadShareholderTmp(final CustMechShareholderTmp anShareholderTmp,
     ...
     * 添加修改代录流水
     */
    public CustMechShareholderTmp saveSaveInsteadShareholderTmp(final CustMechShareholderTmp anShareholderTmp,
            final Long anInsteadRecordId, final String anFileList) {
        BTAssert.notNull(anShareholderTmp, "公司股东流水信息不允许为空！");
    ...
    public int saveCancelInsteadShareholderTmp(final Long anId, final Long anInsteadRecordId) {
        ...
        if (StringUtils.equals(shareholderTmp.getTmpType(), CustomerConstants.TMP_TYPE_INSTEAD) == false) {
            throw new BytterTradeException("流水信息类型不正确,不可撤销.");
        }
    ...
    public CustInsteadRecord addInsteadRecord(final Map<String, Object> anParam, final Long anInsteadRecordId) {
        ...
        COMMA_PATTERN.splitAsStream(tempTmpIds).map(Long::valueOf)
                .forEach(tmpId -> saveShareholderTmpParentIdAndStatus(tmpId, insteadRecord.getId(),
    ...
    private CustInsteadRecord checkInsteadRecord(final Long anInsteadRecordId, final String... anBusinStatus) {
        ...
        if (StringUtils.equals(insteadItem, CustomerConstants.ITEM_SHAREHOLDER) == false) {
    ...
    public CustChangeApply checkChangeApply(final Long anApplyId, final String... anBusinStatus) {
        ...
        if (StringUtils.equals(changeItem, CustomerConstants.ITEM_SHAREHOLDER) == false) {
    ...
    private boolean checkMatchNewChange(final List<Long> anChangeIds, final Long anCustNo) {
        ...
        for (final CustMechShareholderTmp shareholderTmp : ShareholderTmps) {
            ...
            if (StringUtils.equals(shareholderTmp.getTmpOperType(),
