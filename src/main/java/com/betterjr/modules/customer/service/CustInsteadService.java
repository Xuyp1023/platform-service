...
public class CustInsteadService {
    ...
    private final static Pattern DEAL_PASSWORD_PATTERN = Pattern
     ...
     * PC端发起代录申请
     */
    public CustInsteadApply addOpenAccountInsteadApply(final String anCustName, final Long anOperId,
        ...
        Map<String, Object> anMap = QueryTermBuilder.newInstance().put("insteadType", insteadType)
                .put("insteadItems", insteadItems).build();
     ...
     * 代录申请 - 修改申请
     */
    public CustInsteadApply saveInsteadApply(final Map<String, Object> anParam, final Long anApplyId,
            final String anFileList) {
        ...
        if (StringUtils.equals(tempCustInsteadApply.getBusinStatus(),
                CustomerConstants.INSTEAD_APPLY_STATUS_AUDIT_REJECT) == false) {
            throw new BytterTradeException("代录申请状态不允许修改！");
        }

        final CustInsteadApply custInsteadApply = insteadApplyService.saveCustInsteadApply(tempCustInsteadApply,
                anFileList);
     ...
     * 代录申请列表 所有
     */
    public Page<CustInsteadApply> queryInsteadApplyList(final Map<String, Object> anParam, final int anFlag,
     ...
     * 代录拥有列表
     */
    public Page<CustInsteadApply> queryInsteadApplyOwnList(final Map<String, Object> anParam, final int anFlag,
            final int anPageNum, final int anPageSize) {
        BTAssert.notEmpty(anParam, "查询参数不允许为空！");
     ...
     * 代录申请列表 待审核 待录入 复核驳回
     */
    public Page<CustInsteadApply> queryInsteadApplyAuditList(final Map<String, Object> anParam, final int anFlag,
            final int anPageNum, final int anPageSize) {
        BTAssert.notEmpty(anParam, "查询参数不允许为空！");
     ...
     * 代录申请列表 待复核
     */
    public Page<CustInsteadApply> queryInsteadApplyReviewList(final Map<String, Object> anParam, final int anFlag,
            final int anPageNum, final int anPageSize) {
        BTAssert.notEmpty(anParam, "查询参数不允许为空！");
     ...
     * 代录申请列表 待确认
     */
    public Page<CustInsteadApply> queryInsteadApplyConfirmList(final Map<String, Object> anParam, final int anFlag,
            final int anPageNum, final int anPageSize) {
        BTAssert.notEmpty(anParam, "查询参数不允许为空！");
    ...
    public CustInsteadApply saveAuditPassInsteadApply(final Long anId, final String anReason) {
        ...
        final CustInsteadApply insteadApply = saveInsteadApplyStatus(anId,
                CustomerConstants.INSTEAD_APPLY_STATUS_AUDIT_PASS);

        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADAPPLY, CustomerConstants.AUDIT_STEP_AUDIT, anId,
                CustomerConstants.AUDIT_RESULT_PASS, StringUtils.isNotBlank(anReason) ? anReason : "同意", null,
    ...
    public CustInsteadApply saveSubmitReviewInsteadApply(final Long anId) {
        ...
        for (final CustInsteadRecord insteadRecord : insteadRecords) {
            ...
            if (StringUtils.equals(insteadRecord.getBusinStatus(),
                    CustomerConstants.INSTEAD_RECORD_STATUS_REVIEW_REJECT)) {
        ...
        if (includeReviewRejectFlag) {
            ...
            addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADAPPLY, CustomerConstants.AUDIT_STEP_REVIEW, anId,
                    CustomerConstants.AUDIT_RESULT_PASS, "复核通过", null, insteadApply.getCustNo());
    ...
    public CustInsteadApply saveSubmitConfirmInsteadApply(final Long anId) {
        ...
        final CustInsteadApply tempInsteadApply = checkInsteadApply(anId,
                CustomerConstants.INSTEAD_APPLY_STATUS_REVIEW_PASS);

        final String insteadType = tempInsteadApply.getInsteadType();
        if (StringUtils.equals(insteadType, CustomerConstants.INSTEAD_APPLY_TYPE_OPENACCOUNT) == true) {
        ...
        final List<String> businStatus = Arrays.asList(CustomerConstants.INSTEAD_RECORD_STATUS_CONFIRM_PASS,
    ...
    public CustInsteadApply saveCancelInsteadApply(final Long anId, final String anReason) {
        ...
        final CustInsteadApply insteadApply = saveInsteadApplyStatus(anId,
                CustomerConstants.INSTEAD_APPLY_STATUS_CANCEL);

        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADAPPLY, null, anId, CustomerConstants.AUDIT_RESULT_CANCEL,
                "作费申请", null, insteadApply.getCustNo());
    ...
    public CustInsteadApply saveSubmitTypeInInsteadApply(final Long anId) {
        checkInsteadApply(anId, CustomerConstants.INSTEAD_APPLY_STATUS_AUDIT_PASS,
                CustomerConstants.INSTEAD_APPLY_STATUS_REVIEW_REJECT,
                CustomerConstants.INSTEAD_APPLY_STATUS_CONFIRM_REJECT);
        ...
        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADAPPLY, CustomerConstants.AUDIT_STEP_TYPEIN, anId,
                CustomerConstants.AUDIT_RESULT_PASS, "代录提交", null, insteadApply.getCustNo());
    ...
    public CustInsteadApply saveReviewPassInsteadApply(final Long anId, final String anReason) {
        ...
        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADAPPLY, CustomerConstants.AUDIT_STEP_REVIEW, anId,
                CustomerConstants.AUDIT_RESULT_PASS, anReason, null, insteadApply.getCustNo());
    ...
    public CustInsteadApply saveConfirmPassInsteadApply(final Long anId, final String anReason) {
        ...
        final String insteadType = tempInsteadApply.getInsteadType();
        if (StringUtils.equals(insteadType, CustomerConstants.INSTEAD_APPLY_TYPE_OPENACCOUNT) == false) {
            final List<CustInsteadRecord> insteadRecords = insteadRecordService.selectByProperty("applyId", anId);
            for (final CustInsteadRecord insteadRecord : insteadRecords) {
                final String businStatus = insteadRecord.getBusinStatus();

                if (StringUtils.equals(businStatus,
                        CustomerConstants.INSTEAD_RECORD_STATUS_CONFIRM_PASS) == false) {
                    throw new BytterTradeException(20007, "还有代录项目未确认！");
                }
            }
            for (final CustInsteadRecord insteadRecord : insteadRecords) {
    ...
    public CustInsteadRecord saveReviewPassInsteadRecord(final Long anId, final String anReason) {
        ...
        final CustInsteadRecord insteadRecord = saveInsteadRecordStatus(anId,
                CustomerConstants.INSTEAD_RECORD_STATUS_REVIEW_PASS);

        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADRECORD, CustomerConstants.AUDIT_STEP_REVIEW, anId,
                CustomerConstants.AUDIT_RESULT_PASS, StringUtils.isNotBlank(anReason) ? anReason : "同意",
    ...
    public CustInsteadRecord saveReviewRejectInsteadRecord(final Long anId, final String anReason) {
        ...
        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADRECORD, CustomerConstants.AUDIT_STEP_REVIEW, anId,
                CustomerConstants.AUDIT_RESULT_REJECT, anReason, insteadRecord.getInsteadItem(),
                insteadRecord.getCustNo());
    ...
    public CustInsteadRecord saveConfirmPassInsteadRecord(final Long anId, final String anReason) {
        final CustInsteadRecord tempInsteadRecord = checkInsteadRecord(anId,
                CustomerConstants.INSTEAD_RECORD_STATUS_REVIEW_PASS);
        ...
        addCustAuditLog(CustomerConstants.AUDIT_TYPE_INSTEADRECORD, CustomerConstants.AUDIT_STEP_CONFIRM, anId,
                CustomerConstants.AUDIT_RESULT_PASS, StringUtils.isNotBlank(anReason) ? anReason : "同意",
                insteadRecord.getInsteadItem(), insteadRecord.getCustNo());
     ...
     * 添加审核记录
     */
    public void addCustAuditLog(final String anAuditType, final String anStepNode, final Long anBusinId,
            final String anAuditResult, final String anReason, final String anInsteadItem, final Long anCustNo) {
        final CustAuditLog auditLog = auditLogService.addCustAuditLog(anAuditType, anStepNode, anBusinId, anAuditResult,
                anReason, anInsteadItem, anCustNo);
    ...
    private void fillInsteadRecordByAccountTmp(Long anApplyId, Long anAccountTmpid) {
        // 查询对应insteadRecord
        Map<String, Object> anMap = QueryTermBuilder.newInstance().put("applyId", anApplyId)
                .put("insteadItem", CustomerConstants.ITEM_OPENACCOUNT).build();
    ...
    public void generatePassword(CustOpenAccountTmp anAccountTmp, Map<String, Object> anMap) {
        ...
        // 数据校验
        if (StringUtils.isEmpty(anLoginUserName)) {
            BTAssert.notNull(null, "用户登录名不能为空！");
        }
        if (!StringUtils.equals(anNewLoginPassword, anOkLoginPassword)) {
            BTAssert.notNull(null, "所输入两次登录密码不一致，请检查！");
        }
        if (!StringUtils.equals(anNewDealPassword, anOkDealPassword)) {
    ...
    private void verifyMobileMessage(String anMobile, String anVerifyCode) {
        ...
        if (StringUtils.equals(verifyCode.getVerifiCode(), anVerifyCode)) {} else {
    ...
    public CustInsteadApply addWeChatInsteadApply(final String anCustName, final String anFileList) {
        final String insteadType = CustomerConstants.INSTEAD_APPLY_TYPE_OPENACCOUNT;
        final String insteadItems = "0,0,0,0,0,0,0";
        final CustInsteadApply custInsteadApply = insteadApplyService.addWeChatCustInsteadApply(insteadType, anCustName,
                anFileList);
        insteadRecordService.addInsteadRecord(custInsteadApply, insteadType, insteadItems);
    ...
    public CustInsteadRecord saveActiveOpenAccount(Long anId) {
        ...
        // 调用原有提交操作 已经修改。不需再调用
        // CustInsteadApply anInsteadApply =
        // custInsteadService.saveSubmitConfirmInsteadApply(anInsteadRecord.getApplyId());
