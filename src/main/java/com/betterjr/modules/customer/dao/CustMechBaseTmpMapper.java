...
@BetterjrMapper
public interface CustMechBaseTmpMapper extends Mapper<CustMechBaseTmp>, IVersionMapper {
    @Select("SELECT MAX(N_VERSION) FROM t_cust_mech_base_tmp WHERE L_REF_ID=#{refId}")
     ...
     * 取比当前版本低,并且被使用的流水
     */
    @Select("SELECT MAX(N_VERSION) FROM t_cust_mech_base_tmp WHERE L_REF_ID=#{refId} AND N_VERSION < #{version} AND C_BUSIN_STATUS = '2'")
    @ResultType(Long.class)
    @Override
