...
public class NoticeProvider {
    ...
    public String selectUnreadNoticeSql(final Map<String, Object> anParam) {
        final StringBuilder sql = new StringBuilder(
                "SELECT sn.ID as id, sn.C_SUBJECT as subject, sn.D_PUBLISH_DATE as publishDate, sn.T_PUBLISH_TIME as publishTime, sn.L_CUSTNO as custNo, sn.C_CUSTNAME as custName, snc.L_CUSTNO as receiveCustNo, snc.C_CUSTNAME as receiveCustName ");
        ...
        GTEpublishDate.ifPresent(data -> {
            ...
            if (StringUtils.isBlank(publishDate) == false) {
    ...
    public String selectReadNoticeSql(final Map<String, Object> anParam) {
        ...
        sql.append(
                "LEFT JOIN t_sys_notice_oper sno ON sn.ID = sno.L_NOTICE_ID AND (sno.L_OPERID = #{operId} AND sno.L_CUSTNO = snc.L_CUSTNO) ");
        sql.append(
                "WHERE sn.C_BUSIN_STATUS = '1' AND EXISTS (SELECT cor.* FROM t_cust_operator_relation cor WHERE cor.L_OPERNO = #{operId} AND snc.L_CUSTNO = cor.L_CUSTNO) AND sno.ID IS NOT NULL ");
        final Map<String, Object> param = (Map<String, Object>) anParam.get("param");
        final Optional<Object> custNo = Optional.ofNullable(param.get("custNo"));
        custNo.ifPresent(data -> {
            final String tempCustNo = String.valueOf(data);
